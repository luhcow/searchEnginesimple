...
class BPTree {
 private:
  ...
  std::pair<Node<KeyT, ValT> *, KeyT> splitNode(
      Node<KeyT, ValT> *_node);
 ...
 *    @return Index of _key in _node. If _key is not in _node, return
 * the nearest and smaller index
 */
template <typename KeyT, typename ValT>
inline int BPTree<KeyT, ValT>::keyIndex(Node<KeyT, ValT> *_node,
                                        KeyT _key) {
  int loc = -1;
 ...
 *    @return A pair of leaf and index of given _key. If _key not in
 * B+ tree, the index is the nearest and smaller key than given _key.
 */
template <typename KeyT, typename ValT>
inline std::pair<Node<KeyT, ValT> *, int>
BPTree<KeyT, ValT>::keyIndexInLeaf(KeyT _key) {
  if (root == nullptr) {
 ...
 */
template <typename KeyT, typename ValT>
Node<KeyT, ValT> *BPTree<KeyT, ValT>::splitLeaf(
    Node<KeyT, ValT> *_leaf) {
  ...
  new_leaf->ptr2val.assign(_leaf->ptr2val.begin() + mid,
                           _leaf->ptr2val.end());
  _leaf->key.erase(_leaf->key.begin() + mid, _leaf->key.end());
  _leaf->ptr2val.erase(_leaf->ptr2val.begin() + mid,
                       _leaf->ptr2val.end());
  return new_leaf;
}
 ...
 */
template <typename KeyT, typename ValT>
std::pair<Node<KeyT, ValT> *, KeyT> BPTree<KeyT, ValT>::splitNode(
    Node<KeyT, ValT> *_node) {
  ...
  new_node->key.assign(_node->key.begin() + mid + 1,
                       _node->key.end());
  new_node->ptr2node.assign(_node->ptr2node.begin() + mid + 1,
                            _node->ptr2node.end());
  _node->key.erase(_node->key.begin() + mid, _node->key.end());
  _node->ptr2node.erase(_node->ptr2node.begin() + mid + 1,
                        _node->ptr2node.end());
...
/*
 *    @brief Create index for given _new_node using _index as index.
 * The index will be inserted to _new_node's parent.
 *    @param _new_node: Node we want to create index for.
 *    @param _index: Index of our new node. For leaf node, it should
 * be the first key.
 *    @return void
 */
template <typename KeyT, typename ValT>
void BPTree<KeyT, ValT>::createIndex(Node<KeyT, ValT> *_new_node,
                                     KeyT _index) {
...
template <typename KeyT, typename ValT>
Node<KeyT, ValT>::Node(bool _leaf)
...
void BPTree<KeyT, ValT>::insert(KeyT _key, ValT _val) {
  ...
  if (loc != -1 && leaf->key[loc] == _key) {
    // std::cout << "Key " << _key << " with value " <<
    // *(leaf->ptr2val[loc]) << " is already in B+ tree, overwrite it
    // with new val " << _val << std::endl;
    // *(leaf->ptr2val[loc]) = _val;
    return;
  }
  leaf->key.insert(leaf->key.begin() + loc + 1, _key);
  leaf->ptr2val.insert(leaf->ptr2val.begin() + loc + 1,
                       new ValT(_val));
 ...
 *    @return A ptr to value. If key is not in B+ tree then return
 * nullptr
 */
template <typename KeyT, typename ValT>
ValT *BPTree<KeyT, ValT>::find(KeyT _key) {
  ...
  int loc = pair.second;
  if (loc == -1 || leaf->key[loc] != _key) {
    // std::cout << "Key " << _key << " is not in B+ tree" <<
    // std::endl;
    return nullptr;
  } else {
...
void BPTree<KeyT, ValT>::display() {
  ...
  while (!q.empty()) {
    ...
    while (q_size--) {
      ...
      if (node->leaf) {
        for (auto each : node->ptr2val) std::cout << *each << " ";
      } else {
        for (auto each : node->key) std::cout << each << " ";
        for (auto each : node->ptr2node) q.push(each);
      }
      std::cout << "| ";
    }
    std::cout << std::endl;
