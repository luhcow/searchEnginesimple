...
inline Service::Service(): srpc::RPCService("stronly")
{
...
{
	auto *task = this->create_rpc_client_task("/stronly/Echo", std::move(done));
...
inline WFFuture<std::pair<EchoResponse, srpc::RPCSyncContext>> TRPCClient::async_Echo(const EchoRequest *req)
{
	...
	auto *pr = new WFPromise<RESULT>();
	auto fr = pr->get_future();
	auto *task = this->create_rpc_client_task<EchoResponse>("/stronly/Echo", srpc::RPCAsyncFutureCallback<EchoResponse>);
